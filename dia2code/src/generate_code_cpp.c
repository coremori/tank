...
gen_namespace(batch *b,declaration *nsd)
{
	...
	struct sfmllib_includes mi;
	struct statelib_includes ti;
	memset(&mi,0,sizeof(struct sfmllib_includes));
	memset(&ti,0,sizeof(struct statelib_includes));
        ...
        while ( umla != NULL) {
            ...
            print_include_sfmllib(&mi,umla->key.type);
            print_include_statelib(&ti,umla->key.type);
            umla = umla->next;
        }
        // Methods
	    ...
	    print_include_sfmllib(&mi,umlo->key.attr.type);
	    print_include_statelib(&ti,umlo->key.attr.type);
            umlattrlist tmpa = umlo->key.parameters;
            while (tmpa != NULL) {
                print_include_stdlib(&si,tmpa->key.type);
		print_include_sfmllib(&mi,tmpa->key.type);
		print_include_statelib(&ti,tmpa->key.type);
                tmpa = tmpa->next;
            }
            umlo = umlo->next;
        }
